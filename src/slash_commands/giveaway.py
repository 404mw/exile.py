...
class GiveawayView(View):
	...
	async def end_now(self, button: Button, interaction: Interaction):
		...
		if g and g.message_id:
			...
			if isinstance(channel, nextcord.TextChannel):
				try:
					...
					# build/modify embed
					embed = msg.embeds[0] if msg.embeds else nextcord.Embed(title="🎉 Giveaway Ended 🎉", color=0xFFD700)
					# Update (or add) a Winners field
					winner_text = ", ".join(f"<@{w}>" for w in winners) if winners else "No winners"
					# remove any existing Winner(s) field, then add a fresh one
					fields = [f for f in embed.fields if (f.name is not None and f.name != 'Winner(s)')]
					embed.clear_fields()
					for f in fields:
		...
		await interaction.response.send_message("Giveaway ended and winners picked.", ephemeral=True)
		...
		if channel:
			...
			if isinstance(channel, nextcord.TextChannel):
				...
				if g.message_id:
					try:
						msg = await channel.fetch_message(g.message_id)
						embed = msg.embeds[0] if msg.embeds else nextcord.Embed(title="🎉 Giveaway 🎉", color=0xFFD700)
						fields = [f for f in embed.fields if (f.name is not None and f.name != 'Winner(s)')]
						embed.clear_fields()
						for f in fields:
							if f.name is not None and f.value is not None:
...
class GiveawayCog(commands.Cog):
	...
	async def _schedule_end(self, giveaway: Giveaway):
		...
		if channel and giveaway.message_id and isinstance(channel, nextcord.TextChannel):
			try:
				...
				embed = msg.embeds[0] if msg.embeds else nextcord.Embed(title="🎉 Giveaway Ended 🎉", color=0xFFD700)
				# Remove existing Winner(s) field then add ours
				fields = [f for f in embed.fields if (f.name is not None and f.name != 'Winner(s)')]
				embed.clear_fields()
				for f in fields:
					if f.name is not None and f.value is not None:
						embed.add_field(name=str(f.name), value=str(f.value), inline=f.inline)
				embed.add_field(name='Winner(s)', value=mention_text, inline=False)
	...
	async def giveaway(
		...
		prize: str = SlashOption(description="Prize for the giveaway", required=True),
		duration: str = SlashOption(description="Duration", required=True, choices=["12 hours", "1 day", "2 days", "3 days", "7 days"]),
		...
		message: str = SlashOption(description="Custom message", required=False, default=None)
	):
		# Permission check
		...
		# Build embed
		embed = Embed(title="🎉 Giveaway Started! 🎉", color=0xFFD700)
		embed.add_field(name="Prize", value=prize, inline=False)
		embed.add_field(name="Host", value=host.mention, inline=True)
		embed.add_field(name="Duration", value=duration, inline=True)
		embed.add_field(name="Winners", value=str(winners), inline=True)
		# Placeholder for winners, updated when the giveaway ends or is rerolled
		embed.add_field(name="Winner(s)", value="TBD", inline=False)
		if message:
			embed.add_field(name="Message", value=message, inline=False)
		embed.set_footer(text=f"Ends: {end_time.strftime('%Y-%m-%d %H:%M UTC')}")
